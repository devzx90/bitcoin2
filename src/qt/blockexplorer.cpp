...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::BTC2, nValue);
...
static std::string TimeToString(uint64_t Time)
{
	...
	std::string ReturnValue = timestamp.toString("yyyy-MM-dd hh:mm:ss").toUtf8().data();
	ReturnValue += std::string(" (") + i64tostr(Time) + ')';
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    int64_t Generated = GetBlockValue(pBlock->nHeight);
