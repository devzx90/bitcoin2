                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->labelzBTC2SyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBTC2->setEnabled(false);
        ui->pushButtonMintzBTC2->setToolTip(tr("zBTC2 is currently disabled due to maintenance."));

        ui->pushButtonSpendzBTC2->setEnabled(false);
        ui->pushButtonSpendzBTC2->setToolTip(tr("zBTC2 is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzBTC2_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTC2 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBTC2...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBTC2 in ") +
...
void PrivacyDialog::on_pushButtonSpendzBTC2_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTC2 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBTC2
        sendzBTC2();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBTC2
    sendzBTC2();
}

void PrivacyDialog::on_pushButtonZBTC2Control_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBTC2ControlDialog* zBTC2Control = new ZBTC2ControlDialog(this);
    zBTC2Control->setModel(walletModel);
    zBTC2Control->exec();
}
...
void PrivacyDialog::sendzBTC2()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bitcoin2 Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBTC2</b>";
    ...
    // use mints from zBTC2 selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBTC2ControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBTC2ControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBTC2 was spent successfully update the addressbook with the label
    ...
    // Clear zbtc2 selector in case it was used
    ZBTC2ControlDialog::listSelectedMints.clear();
    ui->labelzBTC2Selected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBTC2 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BTC2, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBTC2 Mint");
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        switch (nCoins) {
            case libzerocoin::CoinDenomination::ZQ_FIVECENTS:
                ...
                break;
            case libzerocoin::CoinDenomination::ZQ_TWENTYCENTS:
            ...
            case libzerocoin::CoinDenomination::ZQ_FIVE:
                ...
                break;
            case libzerocoin::CoinDenomination::ZQ_TWENTY:
            ...
            case libzerocoin::CoinDenomination::ZQ_FIVE_HUNDRED:
                ...
                break;
            case libzerocoin::CoinDenomination::ZQ_TWO_THOUSAND:
...
{
    ui->labelzBTC2SyncStatus->setVisible(fShow);
}
