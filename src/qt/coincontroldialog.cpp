...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // Swift TX Fee
        if (coinControl->useSwiftTX) nPayFee = max(nPayFee, 50000);
        // Allow free?
    ...
    QString toolTip4 = tr("Can vary +/- %1 ubtc2 per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Bitcoin2 address. In tree mode, address is not shown again for direct wallet address outputs
