...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight > 0)
	{
		if(nHeight <= Params().LAST_POW_BLOCK()) nSubsidy = COIN * 5250; // In order to reduce the size of the blockchain database, all the bitcoin balances need to be recreated in much fewer blocks.
		else
		{
			int64_t nMoneySupply = chainActive.Height() >= nHeight ? chainActive[nHeight]->nMoneySupply : chainActive.Tip()->nMoneySupply;
			nSubsidy = CENT * 80;
			if(nMoneySupply + nSubsidy > Params().MaxMoneyOut()) nSubsidy = 0;
		}
	}
	else if(nHeight == 0) nSubsidy = COIN;
	else nSubsidy = 0;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
   ...
   // int64_t nBIP16SwitchTime = 1333238400;
    bool fStrictPayToScriptHash = true; //(pindex->GetBlockTime() >= nBIP16SwitchTime);
