...
const int StakingProtocol2Height = 654929; // TODO: Get rid of SPORK_13_STAKING_PROTOCOL_2 completely and use this instead.
unsigned int nStakeInterval = 15; // Every 15 seconds.
unsigned int nMaxStakingFutureDrift = 40; // Max 40 second future drift for POS.
const int nMaxStakingFutureDriftv3 = 25;
...
std::list<libzerocoin::CoinDenomination> ZerocoinSpendListFromBlock(const CBlock& block)
{
    ...
    for (const CTransaction tx : block.vtx)
	{
        if (!tx.HasZerocoinSpendInputs()) continue;
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zBTC2 with serial %s is already in the block %d\n", __func__,
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, CValidationState& state)
{
    ...
    if (fZerocoinActive) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
			...
			{
				LOCK(cs_main);
				fVerifySignature = !IsInitialBlockDownload() && (GetTime() - chainActive.Tip()->GetBlockTime() < (60*60*24));
			}
	...
	else
	{
		BOOST_FOREACH(const CTxIn& txin, tx.vin)
		{
			...
			std::string PrevOutHash = txin.prevout.hash.ToString();
			if (txin.prevout.n == 0 && PrevOutHash == "7efe9ca02792f6aa2019b410563bc629949999166c3d4073817e157270fb3290")
				return state.DoS(1, error("CheckTransaction() : bad prevout"), REJECT_INVALID, "bad-prevout");
		}
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx,  bool* pfMissingInputs, bool fRejectInsaneFee)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;

	LogPrint("masternode", "%s: tx: %s. chainActive.Height(): %d.\n", __func__, tx.ToString(), chainActive.Height());
	...
	LogPrint("masternode", "%s: rather not work on nonstandard transactions.\n", __func__);
    // Rather not work on nonstandard transactions (unless -testnet/-regtest)
    string reason;
    if (chainActive.Tip()->nHeight >= Params().LAST_POW_BLOCK() && Params().RequireStandard() && !IsStandardTx(tx, reason))
        return state.DoS(0,
	...
	LogPrint("masternode", "%s: checked for conflicts with in-memory transactions.\n", __func__);
    {
        CCoinsView dummy;
        ...
        if(tx.HasZerocoinSpendInputs()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBTC2 spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zBTC2 spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
            CCoinsViewMemPool viewMemPool(pcoinsTip, pool);
			LogPrint("masternode", "%s: view.SetBackend(viewMemPool);\n", __func__);
			...
			LogPrint("masternode", "%s: Bring the best block into scope\n", __func__);
            view.GetBestBlock();
		...
		LogPrint("masternode", "%s: Check for non-standard pay-to-script-hash in inputs\n", __func__);
		...
		LogPrint("masternode", "%s: Don't accept it if it can't get into a block\n", __func__);
        ...
        } else if (!tx.HasZerocoinSpendInputs() && nFees < ::minRelayTxFee.GetFee(nSize)) {
        ...
        if (!CheckInputs(tx, state, view, true, STANDARD_SCRIPT_VERIFY_FLAGS, true, NULL, chainActive.Height() >= StakingProtocol2Height ? Params().Zerocoin_StartHeight() : 0)) {
	...
	LogPrint("masternode", "%s: SyncWithWallets(tx, NULL);\n", __func__);
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool* pfMissingInputs, bool fRejectInsaneFee)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        CAmount nValueIn = 0;
        {
            ...
            // do all inputs exist?
            // Note that this does not check for the presence of actual inputs (see the next check for that),
            // only helps filling in pfMissingInputs (to determine missing vs spent).
            for (const CTxIn txin : tx.vin) {
                if (!view.HaveCoins(txin.prevout.hash)) {
                    if (pfMissingInputs)
...
bool HasTX(int a, int n, const string &theAddress, bool QuickScan)
{
	...
	CBlockIndex* pindex2 = chainActive[n];
	int aPercent = c / 100;
	if (aPercent < 10) aPercent = 10;
		...
		BOOST_FOREACH(CTransaction& tx, block.vtx)
		{
			if (!tx.vout.empty())
			{
				for (unsigned int i = 0; i < tx.vout.size(); i++)
				{
					...
					if (!ExtractDestinations(txout.scriptPubKey, type, addresses, nRequired)) continue;

					BOOST_FOREACH(const CTxDestination& addr, addresses) { if (CBitcoinAddress(addr).ToString() == theAddress) { return true; } }
				}
			}
		}
...
///////////
int ScanTX(const string &theAddress, bool QuickScan)
{
	int returnvalue = 0;
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight > 0)
	{
		if (nHeight <= 101) nSubsidy = COIN;
		else if (nHeight <= Params().LAST_POW_BLOCK())
		{
			if(nHeight >= Params().LAST_POW_BLOCK() - 100) nSubsidy = 0; // Not enough time to mature.
			else
			{
				// In order to reduce the size of the blockchain database, all the bitcoin balances needed to be recreated in much fewer blocks.
				nSubsidy = 1418178770622; // Satoshis. (1684806479500000 (Number of satoshis in circulation on BTC block 507850) - (101 * COIN)) / (Params().LAST_POW_BLOCK() - 202);
				if (nHeight == 110) nSubsidy += 1064; // Remainder. 1188 blocks * 1418178770622 + 101 blocks * 100000000 = 1684806479498936. which is 1064 satoshis less than target.
			}
		}
		else
		{
			...
			if (nHeight >= REWARDFORK_BLOCK)
			{
				...
				nSubsidy /= 2;
				if (nHeight >= NextChange)
				{
					nSubsidy -= (CENT * ((BlocksPerMonth + nHeight - NextChange) / BlocksPerMonth)); 
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	{
        if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) nMasternodeCount = mnodeman.stable_size();
        else nMasternodeCount = mnodeman.size();
    }

    if(nMasternodeCount == 0 && !IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) return 0;
    else return blockValue / 4; // Reward masternodes 25% of each block reward.
}
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks
    // of our head, drop it
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // who's tip is within 72 blocks of ours
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks, int Hcheck)
{
    if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
        if (pvChecks)
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	LogPrint("masternode", "%s: pindex height: %d. moving best block to previous block. balance: %d. chainActive.Height(): %d. nChainWork: %s\n", __func__, pindex->nHeight, pwalletMain ? pwalletMain->GetBalance() : 0, chainActive.Height(), chainActive.Tip()->nChainWork.ToString());
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bitcoin2
...
{
    RenameThread("bitcoin2-scriptch");
...
void RecalculateZBTC2Spent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBTC2 supply
...
bool UpdateZBTC2Supply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
{
	LogPrint("masternode", "ConnectBlock AssertLockHeld\n");
    AssertLockHeld(cs_main);
	...
	LogPrint("masternode", "assert(hashPrevBlock == view.GetBestBlock())\n");
	...
	LogPrint("masternode", "%s - Height: %d. Block Time %d. Hash: %s. Checking for TX overwrites.\n", __func__, pindex->nHeight, block.GetBlockTime(), block.GetHash().ToString());
    BOOST_FOREACH (const CTransaction& tx, block.vtx)
	{
	...
	LogPrint("masternode", "%s - CAmount nFees = 0;.\n", __func__);
	...
	LogPrint("masternode", "%s - Check transactions.\n", __func__);
    for (unsigned int i = 0; i < block.vtx.size(); i++)
	{
        ...
        if (tx.HasZerocoinSpendInputs())
		{
            ...
            for (const CTxIn& txIn : tx.vin)
			{
                ...
                CoinSpend spend = TxInToZerocoinSpend(txIn);
                nValueIn += spend.getDenomination() * CENT;

                //Perform checks on the spend that are based on blockchain context
                if (!ContextualCheckCoinSpend(spend, pindex, txid)) return state.DoS(100, error("%s: Coinspend is not valid in block %s", __func__, block.GetHash().GetHex()));
		...
		{
			if (pindex->nHeight >= StakingProtocol2Height)
			{
				for (unsigned int j = 0; j < tx.vin.size(); j++) {
					const COutPoint& prevout = tx.vin[j].prevout;
					const CCoins* coins = view.AccessCoins(prevout.hash);
					if (!coins || (tx.IsCoinStake() && (pindex->nHeight - coins->nHeight) < Params().COINBASE_MATURITY()) || (coins->nHeight < Params().Zerocoin_StartHeight() && !tx.IsCoinStake()) || !coins->IsAvailable(prevout.n)) {
						return state.DoS(100, error("ConnectBlock() : bad inputs"), REJECT_INVALID, "bad-inputs");
					}
				}
			}
	...
	LogPrint("masternode", "%s - Transactions checked.\n", __func__);
	...
	LogPrint("masternode", "%s - if (pindex->nMint > nExpectedMint)\n", __func__);
    ...
    AccumulatorMap mapAccumulators;
	LogPrint("masternode", "%s - ValidateAccumulatorCheckpoint\n", __func__);
	...
	LogPrint("masternode", "%s - Write undo information to disk\n", __func__);
	...
	LogPrint("masternode", "%s - DatabaseChecksums(mapAccumulators);\n", __func__);
    DatabaseChecksums(mapAccumulators);

    if (fTxIndex && !pblocktree->WriteTxIndex(vPos)) return state.Abort("Failed to write transaction index");
	...
	LogPrint("masternode", "%s - view.SetBestBlock(\n", __func__);
    view.SetBestBlock(pindex->GetBlockHash());
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BTC2 to zBTC2
    if(pwalletMain->isZeromintEnabled()) pwalletMain->AutoZeromint();
...
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, CBlock* pblock, bool &fInvalidFound, bool fAlreadyChecked)
{
    ...
    if (fInvalidFound) CheckForkWarningConditionsOnNewFork(vpindexToConnect.back());
    else CheckForkWarningConditions();
...
bool ActivateBestChain(CValidationState& state, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    do {
		...
		if (pindexFork != pindexNewTip) // if tip changed.
		{
			...
			if (!fInitialDownload) {
				...
				int nBlockEstimate = Checkpoints::GetTotalBlocksEstimate();
				{
					...
					for (CNode* node : vNodes) {
						if (pindexNewTip->nHeight >= (node->nStartingHeight != -1 ? node->nStartingHeight - Params().MaxReorganizationDepth() : nBlockEstimate))
...
{
	LogPrint("masternode", "InvalidateBlock()\n");
    AssertLockHeld(cs_main);
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // ppcoin: compute stake modifier
  		if (pindexNew->nHeight < StakingProtocol2Height)
  		{
...
{
	LogPrint("masternode", "%s - BlockTime: %d. Block hash: %s.\n", __func__, block.GetBlockTime(), block.GetHash().ToString());
	if(block.nNonce > 1000000100 && block.nTime > 1599070879) return state.DoS(50, false, REJECT_INVALID, "too high nNonce");

    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits))
        return state.DoS(50, /*error("CheckBlockHeader() : proof of work failed")*/false,
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (!CheckBlockHeader(block, state, block.IsProofOfWork()))
        return state.DoS(100, /*error("CheckBlock() : CheckBlockHeader failed")*/false,
            REJECT_INVALID, "bad-header", true);

    // Check timestamp
	int64_t BiggerTime = GetAdjustedTime();
	if (GetTime() > BiggerTime) BiggerTime = GetTime();
    if (block.GetBlockTime() > BiggerTime + (block.IsProofOfStake() ? nMaxStakingFutureDrift : 7200))
		return state.DoS(10, error("CheckBlock() : block timestamp too far in the future"), REJECT_INVALID, "ime-too-new");

	LogPrint("masternode", "%s - Time stamp checked ok. BlockTime: %d. Block hash: %s.\n", __func__, block.GetBlockTime(), block.GetHash().ToString());
	...
	LogPrint("masternode", "%s - merkle root checked ok.\n", __func__);
	...
	LogPrint("masternode", "%s - Coinbase output should be empty if proof-of-stake block\n", __func__);
    if (block.IsProofOfStake()) {
	...
	LogPrint("masternode", "%s - swiftTX transaction scanning.\n", __func__);
    // ----------- swiftTX transaction scanning -----------
    ...
    if (block.IsProofOfStake())
	{
		LogPrint("masternode", "%s - if (block.IsProofOfStake()).\n", __func__);
		LOCK(cs_main);
    ...
    for (const CTransaction& tx : block.vtx) {
		LogPrint("masternode", "%s - Check tx: %s.\n", __func__, tx.ToString());
        if (!CheckTransaction(tx, fZerocoinActive, state))
            return error("CheckBlock() : CheckTransaction failed");

        // double check that there are no double spent zBTC2 spends in this block
        if (tx.HasZerocoinSpendInputs())
		{
            for (const CTxIn txIn : tx.vin) {
                if (txIn.IsZerocoinSpend()) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBTC2 serial %s in block\n Block: %s",
	...
	LogPrint("masternode", "%s - transactions fine.\n", __func__);
    unsigned int nSigOps = 0;
    BOOST_FOREACH (const CTransaction& tx, block.vtx) {
        nSigOps += GetLegacySigOpCount(tx);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
	...
	LogPrint("masternode", "%s Start - nHeight: %d. Block: %s. BlockTime: %d\n", __func__, nHeight, block.GetHash().ToString(), block.GetBlockTime());

    //If this is a reorg, check that it is not too deep
    int nMaxReorgDepth = GetArg("-maxreorg", Params().MaxReorganizationDepth());
    if (chainActive.Height() - nHeight >= nMaxReorgDepth)
        return state.DoS(10, error("%s: forked chain older than max reorganization depth (height %d)", __func__, nHeight));
	...
	if (nHeight >= REWARDFORK_BLOCK)
	{
		...
		LogPrint("masternode", "%s - Max 3 blocks with the same time stamp check.\n", __func__);
		for (int i = 0; i < 3; ++i)
		{
			if (aBlockIndex->nTime < block.GetBlockTime())
			{
		...
		LogPrint("masternode", "%s - Max 3 blocks with the same time stamp check done.\n", __func__);
		if (TooManyBlocksWithSameTimeStamp) return state.Invalid(error("%s : too many blocks with this timestamp.", __func__), REJECT_INVALID, "time-too-used");
	}
	...
	LogPrint("masternode", "%s - Checkpoints::CheckBlock\n", __func__);
	...
	LogPrint("masternode", "%s - pcheckpoint = Checkpoints::GetLastCheckpoint();\n", __func__);
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
	...
	LogPrint("masternode", "%s - transactions are finalized.\n", __func__);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	LogPrint("masternode", "%s - Previous block: %s. Block Time %d. Hash: %s.\n", __func__, block.hashPrevBlock.ToString(), block.GetBlockTime(), block.GetHash().ToString());
    ...
    if (block.GetHash() != Params().HashGenesisBlock()) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(0, error("%s : prev block %s not found", __func__, block.hashPrevBlock.ToString().c_str()), 0, "bad-prevblk");
        pindexPrev = (*mi).second;
		LogPrint("masternode", "%s - Previous Height: %d.\n", __func__, pindexPrev->nHeight);
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            //If this "invalid" block is an exact match from the checkpoints, then reconsider it
	...
	LogPrint("masternode", "%s - Checking Work.\n", __func__);
    if (block.GetHash() != Params().HashGenesisBlock() && !CheckWork(block, pindexPrev)) // this makes all the Proof of Stake checks too.
        return state.DoS(100, false);
	LogPrint("masternode", "%s - CheckWork done.\n", __func__);
	...
	LogPrint("masternode", "%s - if ((!fAlreadyCheckedBlock\n", __func__);
	...
	if(block.IsProofOfStake())
	{
		...
		LogPrint("masternode", "%s - isBlockFromFork: %d", __func__, isBlockFromFork);

		// Coin stake
		CTransaction &stakeTxIn = block.vtx[1];

		// Inputs
		std::vector<CTxIn> StakingInputs;
		for (const CTxIn& stakeIn : stakeTxIn.vin)
		{
			StakingInputs.push_back(stakeIn);
		}
		if(StakingInputs.empty()) return error("%s: No staking inputs.", __func__);

		const CCoinsViewCache aCoinsCache(pcoinsTip);
		for (const CTransaction& tx : block.vtx)
		{
			for (const CTxIn& in : tx.vin)
			{
				...
				for (const CTxIn& BTC2In : StakingInputs) {
					if (BTC2In.prevout == in.prevout) {
		...
		// Check timestamp
		LogPrint("masternode", "%s - Check timestamp", __func__);
		if (pindex->nHeight >= StakingProtocol2Height && (block.GetBlockTime() % nStakeInterval != 0 || pindexPrev->GetBlockTime() > block.GetBlockTime() + nMaxPastTimeSecs))
		...
		if (isBlockFromFork)
		{
			...
			CBlock bl;
			LogPrint("masternode", "%s - block is from fork. ReadBlockFromDisk", __func__);
			...
			LogPrint("masternode", "%s -Go backwards on the forked chain up to the split", __func__);
			while (!chainActive.Contains(prev))
			{
				...
				for (const CTransaction &t : bl.vtx)
				{
					for (const CTxIn &in : t.vin)
					{
						...
						for (const CTxIn &stakeIn : StakingInputs)
						{
				...
				// Prev block
				prev = prev->pprev;
				if (prev == NULL || !ReadBlockFromDisk(bl, prev))
					// Previous block not on disk
					return error("%s: previous block %s not on disk", __func__, prev->GetBlockHash().GetHex());
			}
		}
		else // let's check if the staking inputs were spent on the main chain and are mature
		{
			for (const CTxIn& in : stakeTxIn.vin) {
				const CCoins* coin = aCoinsCache.AccessCoins(in.prevout.hash);
				...
				else if (!coin->IsAvailable(in.prevout.n)) {
					// Coins not available
					return error("%s: coin stake inputs already spent in main chain", __func__);
				}
				else if (pindex->nHeight >= StakingProtocol2Height)
				{
	...
	LogPrint("masternode", "%s - Write block to history file", __func__);
	...
	LogPrint("masternode", "%s - return true", __func__);
    return true;
}
...
{
	LogPrint("masternode", "%s - From: %s. Block Time %d. Hash: %s.\n", __func__, pfrom != NULL ? pfrom->addr.ToStringIP() : "myself", pblock->GetBlockTime(), pblock->GetHash().ToString());
    ...
    if (pblock->GetHash() != Params().HashGenesisBlock() && pfrom != NULL)
	{
        ...
        if (mi == mapBlockIndex.end()) {
			...
			LogPrint("masternode", "ProcessNewBlock - The peer's block previous to this block is not one of our previous blocks. That node is on a different or longer chain. chainActive.Height(): %d. nChainWork: %s\n", chainActive.Height(), chainActive.Tip()->nChainWork.ToString());
            return false;
        }
    }
	...
	LogPrint("masternode", "%s - CheckBlockSignature.\n", __func__);
...

    {
		...
		LogPrint("masternode", "%s - CheckBlockIndex();.\n", __func__);
		CheckBlockIndex();
		if (!ret)
		{
			// Check spamming
			LogPrint("masternode", "%s - Check spamming.\n", __func__);
	...
	LogPrint("masternode", "%s - ActivateBestChain()\n", __func__);
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
	...
	LogPrintf("%s : CCoinsViewCache viewNew(pcoinsTip);\n", __func__);
    CCoinsViewCache viewNew(pcoinsTip);
    CBlockIndex indexDummy(block);
    indexDummy.pprev = pindexPrev;
    indexDummy.nHeight = pindexPrev->nHeight + 1;
	LogPrintf("%s : if (!ContextualCheckBlockHeader()\n", __func__);
    // NOTE: CheckBlockHeader is called by CheckBlock
    if (!ContextualCheckBlockHeader(block, state, pindexPrev))
        return false;

	LogPrintf("%s : if (!CheckBlock()\n", __func__);
    if (!CheckBlock(block, state, fCheckPOW, fCheckMerkleRoot))
        return false;

	LogPrintf("%s : if (!ContextualCheckBlock()\n", __func__);
    if (!ContextualCheckBlock(block, state, pindexPrev))
        return false;

	LogPrintf("%s : if (!ConnectBlock()\n", __func__);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PIVX 3.0.6: AddTimeData(pfrom->addr, nTime);
    }
        ...
        if (!tx.HasZerocoinSpendInputs() && AcceptToMemoryPool(mempool, state, tx, &fMissingInputs, false)) {
            mempool.check(pcoinsTip);
            RelayTransaction(tx);
        ...
        } else if (tx.HasZerocoinSpendInputs() && AcceptToMemoryPool(mempool, state, tx, &fMissingZerocoinInputs, false)) {
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
		...
		else
		{
            ...
            if (!mapBlockIndex.count(block.GetHash()))
			{
                ...
                int nDoS;
                if(state.IsInvalid(nDoS)) {
					LogPrint("masternode", "Rejected a block: %s peer=%d\n", state.GetRejectReason(), pfrom->id);
                    pfrom->PushMessage("reject", strCommand, state.GetRejectCode(),
                                       state.GetRejectReason().substr(0, MAX_REJECT_MESSAGE_LENGTH), inv.hash);
                    if(nDoS > 0) {
                        TRY_LOCK(cs_main, lockMain);
...
int ActiveProtocol()
{
   //if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT)) return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	if (IsSporkActive(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2)) return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;
