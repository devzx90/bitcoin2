...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 BTC2
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 BTC2 over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 BTC2+1 ( bit on if present )
            // bit 1 - 1000 BTC2+1
            // bit 2 - 100 BTC2+1
            // bit 3 - 10 BTC2+1
            // bit 4 - 1 BTC2+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 BTC2.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 1000 BTC2.");
                    } else {
                    ...
                    if (useIX) {
                        strFailReason += " " + _("Swift TX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
...
bool CWallet::FinishCoinStake(CMutableTransaction& txNew, CAmount nCredit)
{
	...
	if ((txNew.vout.size() == 3 || txNew.vout.size() == 4) && txNew.vout[1].nValue == 0 && txNew.vout[2].nValue == 0) { // Without the == 0 checks this could erase the masternode reward.
		...
		if (RewardCutAmount != 0)
		{
			...
			if(RewardCutAmount <= nReward && (RewardCutAmount > MINRELAYFEE || RewardCutAmount == 3)) // 3 is a special case that sends a third of the block reward amount.
			{
				txNew.vout[2].nValue = (RewardCutAmount == 3 ? (nReward / RewardCutAmount) : RewardCutAmount);
	...
	// Sign
	if (!SignSignature(*this, scriptPubKeyKernel, txNew, 0, SIGHASH_ALL)) return error("CreateCoinStake : failed to sign coinstake");
...
unsigned int CWallet::ComputeTimeSmart(const CWalletTx& wtx) const
{
    ...
    if (wtx.hashBlock != 0) {
        if (mapBlockIndex.count(wtx.hashBlock)) {
            ...
            {
                // Tolerate times up to the last timestamp in the wallet not more than 40 seconds into the future
                int64_t latestTolerated = latestNow + 40; // PIVX has 300 secs here
...
void CWallet::AutoZeromint()
{
    ...
    if (nBalance < 50 * CENT) return;

    // Percentage of zBTC2 we already have
    double dPercentage = 100 * (double)nZerocoinBalance / (double)(nZerocoinBalance + nBalance);

    // Check if minting is actually needed
    if(dPercentage >= nZeromintPercentage) return;

    // zBTC2 amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zBTC2 amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / CENT;

    // Use the biggest denomination smaller than the needed zBTC2 We'll only mint exact denomination to make minting faster.
    // Exception: for big amounts use ZQ_262625 to create all
    // possible denominations to avoid having ZQ_TWO_THOUSAND denominations only.
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough BTC2 to mint this denomination

    if (nPreferredDenom > 0)
	{
        if (nToMintAmount >= nPreferredDenom) nToMintAmount = nPreferredDenom;  // Enough coins => mint preferred denomination
        else nToMintAmount = 0; // Not enough coins => do nothing and wait for more coins
    }

    if (nToMintAmount >= ZQ_262625){
        nMintAmount = ZQ_262625;
    } else if (nToMintAmount >= libzerocoin::CoinDenomination::ZQ_TWO_THOUSAND){
        nMintAmount = libzerocoin::CoinDenomination::ZQ_TWO_THOUSAND;
    ...
    } else if (nToMintAmount >= libzerocoin::CoinDenomination::ZQ_TWENTY){
        nMintAmount = libzerocoin::CoinDenomination::ZQ_TWENTY;
    ...
    } else if (nToMintAmount >= libzerocoin::CoinDenomination::ZQ_TWENTYCENTS){
        nMintAmount = libzerocoin::CoinDenomination::ZQ_TWENTYCENTS;
    } else if (nToMintAmount >= libzerocoin::CoinDenomination::ZQ_FIVECENTS){
        nMintAmount = libzerocoin::CoinDenomination::ZQ_FIVECENTS;
    } else {
        nMintAmount = 0;
    }
    ...
    CWalletTx wtx;
        ...
        string strError = pwalletMain->MintZerocoin(nMintAmount*CENT, wtx, vMints);
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zBTC2. Current percentage of zBTC2: %lf%%\n",
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //any change that is less than 10000 satoshis will be ignored and given as an extra fee
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to BTC2
    CAmount nChange = nValueIn - nTotalValue; // Fee already accounted for in nTotalValue
    if (nChange >= 10000 && !isZCSpendChange) {
    ...
    // Sign if these are bitcoin2 outputs - NOTE that zBTC2 outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZBTC2_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZBTC2_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZBTC2_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZBTC2_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a Bitcoin2 transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZBTC2_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZBTC2_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zBTC2
                ...
                receipt.SetStatus(_("The coin spend has been used"), ZBTC2_SPENT_USED_ZBTC2);
                return false;
            }
        }
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZBTC2_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZBTC2_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZBTC2_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < CENT * 5) {
        receipt.SetStatus(_("Value is below the the smallest available denomination (= 0.05) of zBTC2"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZBTC2_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBTC2 transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zBTC2 mints to use in this spend
        std::map<libzerocoin::CoinDenomination, CAmount> DenomMap = GetMyZerocoinDistribution();
        vSelectedMints = SelectMintsFromList(nValue, nValueSelected, nMaxSpends, fMinimizeChange,
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to bitcoin2 address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZBTC2_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZBTC2_SPEND_OKAY); // Everything okay
...
void CWallet::ZBTC2BackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozbtc2backup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbtc2backup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozbtc2backup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZBTC2_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZBTC2_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZBTC2_ERASE_SPENDS_FAILED);
            }
    ...
    receipt.SetStatus("Spend Successful", ZBTC2_SPEND_OKAY);  // When we reach this point spending zBTC2 was successful
