...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bitcoin2.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bitcoin2d.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BTC2 and zBTC2 money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8333, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BTC2/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BTC2/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bitcoin2, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, net)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BTC2/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Bitcoin2 specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8333"));
    ...
    strUsage += HelpMessageOpt("-zeromintpercentage=<n>", strprintf(_("Percentage of automatically minted Zerocoin  (10-100, default: %u)"), 10)); 5,

    strUsage += HelpMessageOpt("-preferredDenom=<n>", strprintf(_("Preferred Denomination for automatically minted Zerocoins (in cents) (5/20/100/500/2000/10000/50000/200000), 0 for no preference. default: %u)"), 0));
    strUsage += HelpMessageOpt("-backupzbtc2=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBTC2 minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizebitcoin2amount=<n>     " + strprintf(_("Keep N BTC2 anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Bitcoin 2 Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Bitcoin2 is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitcoin 2 Core is shutting down."));
...
#endif
    // Make sure only a single Bitcoin2 process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcoin 2 Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BTC2 specific: zerocoin and spork DB's
                ...
                // BTC2: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitcoin 2 Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Bitcoin 2 Core to complete") << "\n";
        ...
        bool fEnableZBTC2Backups = GetBoolArg("-backupzbtc2", true);
        pwalletMain->setZBTC2AutoBackups(fEnableZBTC2Backups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
    ...
    if (nPreferredDenom != 0 && libzerocoin::IntToZerocoinDenomination(nPreferredDenom) == libzerocoin::CoinDenomination::ZQ_ERROR){
...
//    }
//
//    nAnonymizeBitcoin2Amount = GetArg("-anonymizebitcoin2amount", 0);
//    if (nAnonymizeBitcoin2Amount > 999999) nAnonymizeBitcoin2Amount = 999999;
//    if (nAnonymizeBitcoin2Amount < 2) nAnonymizeBitcoin2Amount = 2;
    ...
    LogPrintf("Anonymize BTC2 Amount %d\n", nAnonymizeBitcoin2Amount);
    ...
    /* Denominations
       ...
       For example:
       1BTC2+1000 == (.1BTC2+100)*10
       10BTC2+10000 == (1BTC2+1000)*10
    */
