...
// Zerocoin denomination which creates exactly one of each of the denominations
static const int ZQ_262625 = 262625;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 BTC2 at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBTC2 send
enum ZerocoinSpendStatus {
    ZBTC2_SPEND_OKAY = 0,                            // No error
    ZBTC2_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBTC2_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBTC2_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBTC2_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBTC2_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBTC2_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBTC2_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBTC2_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBTC2_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBTC2_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBTC2_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBTC2_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBTC2_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBTC2_SPENT_USED_ZBTC2 = 14,                      // Coin has already been spend
    ZBTC2_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZBTC2AutoBackups(bool fEnabled)
    {
	...
	CAmount GetSpendableCoinsOfAddress(CBitcoinAddress &theAddress, CCoinControl* ToCoinControl, int minConfirmations = 0, CAmount StopAtAmount = Params().MaxMoneyOut());
	bool SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet) const;

    /// Get 1000 BTC2 output and keys which can be used for the Masternode
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = false, CAmount nFeePay = 0, std::string theChangeAddress = "");
