...
- [Building Bitcoin2](#building-bitcoin2)
...
Clone the git repositories for bitcoin2 and gitian and then checkout the bitcoin2 version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoin2 repository
...
To build Bitcoin2 (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoin2 repository.
...
```bash
    ...
    From https://github.com/bitcoin2-crypto/bitcoin2
...
For example:
```bash
URL=https://github.com/crowning-/bitcoin2.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoin2-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoin2-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoin2-build.assert
```
...
[bitcoin2/gitian.sigs](https://github.com/bitcoin2-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
